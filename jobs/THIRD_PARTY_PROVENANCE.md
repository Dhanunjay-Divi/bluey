...
| [`santifer/career-ops`](https://github.com/santifer/career-ops) | `267dfb707987` | MIT | Rechecked at the pinned revision; original port lineage was `01bf8b469ad5`. Adapted public ATS provider, host-pinning, bounded pagination, retry, normalization, exact dedupe, source-trust reasons, description-fingerprint cross-listing signals, and the requirement to read back controlled form values before advancing. Bluey's durable Auto-submit authority and form read-back are original implementations; no selector set, source file, wording, or asset was copied. |
...
| `automation/src/form-readback.ts` | Form-registration guidance rechecked in `career-ops` | Original provider-neutral Bluey expectations for text, select, boolean, and file controls, verified after fill and immediately before Submit without logging candidate values. |
