...
int bluey_resampler_init(BlueyResampler *resampler, double source_rate) {
    ...
    for (int phase_index = 0; phase_index < BLUEY_RESAMPLER_PHASES; phase_index++) {
        ...
        for (int tap = 0; tap < BLUEY_RESAMPLER_TAPS; tap++) {
            double distance =
                (double)(tap - BLUEY_RESAMPLER_HALF_TAPS + 1) - fractional;
            double window_phase =
                (double)tap / (double)(BLUEY_RESAMPLER_TAPS - 1);
            double window = 0.42
        ...
        for (int tap = 0; tap < BLUEY_RESAMPLER_TAPS; tap++) {
            resampler->coefficients[phase_index][tap] =
                (float)(
                    (double)resampler->coefficients[phase_index][tap]
                    / coefficient_sum
                );
        }
    }
...
int bluey_resampler_push(
    ...
    resampler->latest_input_index += 1;
    resampler->ring[
        (size_t)resampler->latest_input_index % BLUEY_RESAMPLER_RING_CAPACITY
    ] = sanitize_sample(sample);

    while (
        resampler->next_output_time + BLUEY_RESAMPLER_HALF_TAPS
        ...
        if (weighted_sum > 1.0) {
            weighted_sum = 1.0;
        } else if (weighted_sum < -1.0) {
            weighted_sum = -1.0;
        }
        int32_t scaled = weighted_sum >= 0.0
            ? (int32_t)(weighted_sum * 32767.0 + 0.5)
            : (int32_t)(weighted_sum * 32768.0 - 0.5);
