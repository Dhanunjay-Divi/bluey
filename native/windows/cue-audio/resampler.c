...
int bluey_resampler_init(BlueyResampler *resampler, double source_rate) {
    ...
    /* Precompute all fractional phases so live capture performs no trigonometry. */
...
int bluey_resampler_push(
    ...
    resampler->latest_input_index += 1;
    resampler->ring[(size_t)resampler->latest_input_index % BLUEY_RESAMPLER_RING_CAPACITY] =
