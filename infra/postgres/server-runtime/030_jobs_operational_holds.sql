...
CREATE OR REPLACE FUNCTION validate_jobs_operational_hold_event()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.event_ref !~ '^event-[0-9a-f]{64}$'
     OR NEW.event_sha256 !~ '^[0-9a-f]{64}$'
     OR NEW.canonical_event_base64url !~ '^[A-Za-z0-9_-]+$'
     OR (NEW.reason_ref IS NOT NULL
       AND NEW.reason_ref !~ '^[A-Za-z0-9][A-Za-z0-9._:/@+-]{0,191}$') THEN
    RAISE EXCEPTION 'invalid operational hold event';
  END IF;
  IF NEW.revision_no > 1 AND NOT EXISTS (
    SELECT 1
...
CREATE OR REPLACE FUNCTION reject_jobs_operational_hold_event_mutation()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION validate_jobs_operational_hold_head_insert()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.scope_ref !~ '^scope-[0-9a-f]{64}$'
     OR NEW.current_event_ref !~ '^event-[0-9a-f]{64}$'
    ...
    RAISE EXCEPTION 'invalid operational hold head';
  END IF;
  RETURN NEW;
END;
$$;

CREATE OR REPLACE FUNCTION enforce_jobs_operational_hold_head_monotonic()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.capability <> OLD.capability
    ...
    RAISE EXCEPTION 'operational hold head must advance exactly one revision';
  END IF;
  RETURN NEW;
END;
$$;

CREATE OR REPLACE FUNCTION reject_jobs_operational_hold_head_delete()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_jobs_operational_hold_events_validate_insert
  ON jobs_operational_hold_events;
CREATE TRIGGER trg_jobs_operational_hold_events_validate_insert
BEFORE INSERT ON jobs_operational_hold_events
FOR EACH ROW EXECUTE FUNCTION validate_jobs_operational_hold_event();
DROP TRIGGER IF EXISTS trg_jobs_operational_hold_events_no_update
  ON jobs_operational_hold_events;
CREATE TRIGGER trg_jobs_operational_hold_events_no_update
BEFORE UPDATE ON jobs_operational_hold_events
FOR EACH ROW EXECUTE FUNCTION reject_jobs_operational_hold_event_mutation();
DROP TRIGGER IF EXISTS trg_jobs_operational_hold_events_no_delete
  ON jobs_operational_hold_events;
CREATE TRIGGER trg_jobs_operational_hold_events_no_delete
BEFORE DELETE ON jobs_operational_hold_events
FOR EACH ROW EXECUTE FUNCTION reject_jobs_operational_hold_event_mutation();

DROP TRIGGER IF EXISTS trg_jobs_operational_hold_heads_validate_insert
  ON jobs_operational_hold_heads;
CREATE TRIGGER trg_jobs_operational_hold_heads_validate_insert
BEFORE INSERT ON jobs_operational_hold_heads
FOR EACH ROW EXECUTE FUNCTION validate_jobs_operational_hold_head_insert();
DROP TRIGGER IF EXISTS trg_jobs_operational_hold_heads_monotonic
  ON jobs_operational_hold_heads;
CREATE TRIGGER trg_jobs_operational_hold_heads_monotonic
BEFORE UPDATE ON jobs_operational_hold_heads
FOR EACH ROW EXECUTE FUNCTION enforce_jobs_operational_hold_head_monotonic();
DROP TRIGGER IF EXISTS trg_jobs_operational_hold_heads_no_delete
  ON jobs_operational_hold_heads;
CREATE TRIGGER trg_jobs_operational_hold_heads_no_delete
BEFORE DELETE ON jobs_operational_hold_heads
FOR EACH ROW EXECUTE FUNCTION reject_jobs_operational_hold_head_delete();
