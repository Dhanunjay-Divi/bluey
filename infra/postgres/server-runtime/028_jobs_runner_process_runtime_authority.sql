-- Target: PostgreSQL
-- Process-scoped cloud-runner runtime authority. Keep this structurally paired
-- with SQLite migration 050_jobs_runner_process_runtime_authority.sql.
...
CREATE OR REPLACE FUNCTION bluey_jobs_reject_runner_process_runtime_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $fn$
BEGIN
  RAISE EXCEPTION 'runner process runtime authority is immutable';
END;
$fn$;

DROP TRIGGER IF EXISTS trg_jobs_runner_process_runtime_grants_no_update
  ON jobs_runner_process_runtime_grants;
CREATE TRIGGER trg_jobs_runner_process_runtime_grants_no_update
BEFORE UPDATE ON jobs_runner_process_runtime_grants
FOR EACH ROW EXECUTE FUNCTION bluey_jobs_reject_runner_process_runtime_mutation();

DROP TRIGGER IF EXISTS trg_jobs_runner_process_runtime_grants_no_delete
  ON jobs_runner_process_runtime_grants;
CREATE TRIGGER trg_jobs_runner_process_runtime_grants_no_delete
BEFORE DELETE ON jobs_runner_process_runtime_grants
FOR EACH ROW EXECUTE FUNCTION bluey_jobs_reject_runner_process_runtime_mutation();

DROP TRIGGER IF EXISTS trg_jobs_runner_process_runtime_grant_revocations_no_update
  ON jobs_runner_process_runtime_grant_revocations;
CREATE TRIGGER trg_jobs_runner_process_runtime_grant_revocations_no_update
BEFORE UPDATE ON jobs_runner_process_runtime_grant_revocations
FOR EACH ROW EXECUTE FUNCTION bluey_jobs_reject_runner_process_runtime_mutation();

DROP TRIGGER IF EXISTS trg_jobs_runner_process_runtime_grant_revocations_no_delete
  ON jobs_runner_process_runtime_grant_revocations;
CREATE TRIGGER trg_jobs_runner_process_runtime_grant_revocations_no_delete
BEFORE DELETE ON jobs_runner_process_runtime_grant_revocations
FOR EACH ROW EXECUTE FUNCTION bluey_jobs_reject_runner_process_runtime_mutation();

DROP TRIGGER IF EXISTS trg_jobs_runner_process_runtime_bindings_no_update
  ON jobs_runner_process_runtime_bindings;
CREATE TRIGGER trg_jobs_runner_process_runtime_bindings_no_update
BEFORE UPDATE ON jobs_runner_process_runtime_bindings
FOR EACH ROW EXECUTE FUNCTION bluey_jobs_reject_runner_process_runtime_mutation();

DROP TRIGGER IF EXISTS trg_jobs_runner_process_runtime_bindings_no_delete
  ON jobs_runner_process_runtime_bindings;
CREATE TRIGGER trg_jobs_runner_process_runtime_bindings_no_delete
BEFORE DELETE ON jobs_runner_process_runtime_bindings
FOR EACH ROW EXECUTE FUNCTION bluey_jobs_reject_runner_process_runtime_mutation();

DROP TRIGGER IF EXISTS trg_jobs_execution_lease_process_runtime_bindings_no_update
  ON jobs_execution_lease_process_runtime_bindings;
CREATE TRIGGER trg_jobs_execution_lease_process_runtime_bindings_no_update
BEFORE UPDATE ON jobs_execution_lease_process_runtime_bindings
FOR EACH ROW EXECUTE FUNCTION bluey_jobs_reject_runner_process_runtime_mutation();

DROP TRIGGER IF EXISTS trg_jobs_execution_lease_process_runtime_bindings_no_delete
  ON jobs_execution_lease_process_runtime_bindings;
CREATE TRIGGER trg_jobs_execution_lease_process_runtime_bindings_no_delete
BEFORE DELETE ON jobs_execution_lease_process_runtime_bindings
FOR EACH ROW EXECUTE FUNCTION bluey_jobs_reject_runner_process_runtime_mutation();
