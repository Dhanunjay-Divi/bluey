...
CREATE TABLE IF NOT EXISTS jobs_runner_legacy_inventory_authorities (
  ...
  authority_generation            BIGINT NOT NULL UNIQUE
...
CREATE OR REPLACE FUNCTION reject_jobs_runner_legacy_inventory_authority_mutation()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'runner legacy inventory authority is append-only';
END;
$$;

DROP TRIGGER IF EXISTS trg_jobs_runner_legacy_inventory_authorities_no_update
  ON jobs_runner_legacy_inventory_authorities;
CREATE TRIGGER trg_jobs_runner_legacy_inventory_authorities_no_update
BEFORE UPDATE ON jobs_runner_legacy_inventory_authorities
FOR EACH ROW EXECUTE FUNCTION reject_jobs_runner_legacy_inventory_authority_mutation();

DROP TRIGGER IF EXISTS trg_jobs_runner_legacy_inventory_authorities_no_delete
  ON jobs_runner_legacy_inventory_authorities;
CREATE TRIGGER trg_jobs_runner_legacy_inventory_authorities_no_delete
...
CREATE TABLE IF NOT EXISTS jobs_runner_volume_fleet_state (
  ...
  purge_generation                BIGINT NOT NULL DEFAULT 0
    CHECK(purge_generation >= 0),
  tombstone_generation            BIGINT NOT NULL DEFAULT 0
    CHECK(tombstone_generation >= 0),
  destruction_generation          BIGINT NOT NULL DEFAULT 0
    CHECK(destruction_generation >= 0),
  legacy_reconciliation_generation BIGINT NOT NULL DEFAULT 0
    CHECK(legacy_reconciliation_generation >= 0),
  storage_attestation_generation  BIGINT NOT NULL DEFAULT 0
    CHECK(storage_attestation_generation >= 0),
  storage_attestation_count       BIGINT NOT NULL DEFAULT 0
  ...
  legacy_inventory_generation      BIGINT NOT NULL DEFAULT 0
  ...
  legacy_inventory_root_count      BIGINT CHECK(legacy_inventory_root_count >= 0),
  ...
  unresolved_legacy_volume_count  BIGINT NOT NULL DEFAULT 0
    CHECK(unresolved_legacy_volume_count >= 0),
  cutover_enrollment_generation   BIGINT CHECK(cutover_enrollment_generation >= 0),
  cutover_purge_generation        BIGINT CHECK(cutover_purge_generation >= 0),
  cutover_tombstone_generation    BIGINT CHECK(cutover_tombstone_generation >= 0),
  cutover_destruction_generation  BIGINT CHECK(cutover_destruction_generation >= 0),
  cutover_legacy_reconciliation_generation BIGINT
    CHECK(cutover_legacy_reconciliation_generation >= 0),
  cutover_storage_attestation_generation BIGINT
    CHECK(cutover_storage_attestation_generation >= 0),
  cutover_storage_attestation_count BIGINT
    CHECK(cutover_storage_attestation_count >= 0),
  cutover_storage_attestation_set_sha256 TEXT,
  cutover_legacy_inventory_generation BIGINT
  ...
  cutover_legacy_inventory_root_count BIGINT
    CHECK(cutover_legacy_inventory_root_count >= 0),
  cutover_legacy_inventory_root_set_sha256 TEXT,
  cutover_non_destroyed_volume_count BIGINT CHECK(cutover_non_destroyed_volume_count >= 0),
  cutover_destruction_count       BIGINT CHECK(cutover_destruction_count >= 0),
  cutover_unresolved_legacy_volume_count BIGINT
...
INSERT INTO jobs_runner_volume_fleet_state (
...
CREATE TABLE IF NOT EXISTS jobs_runner_volume_admission_grants (
  ...
  issued_fleet_generation  BIGINT NOT NULL CHECK(issued_fleet_generation >= 0),
...
CREATE TABLE IF NOT EXISTS jobs_runner_volumes (
  ...
  enrollment_generation         BIGINT NOT NULL CHECK(enrollment_generation >= 1),
  required_tombstone_generation BIGINT NOT NULL CHECK(required_tombstone_generation >= 0),
  reconciled_tombstone_generation BIGINT NOT NULL DEFAULT 0
  ...
  instance_lease_expires_at_ms  BIGINT,
  legacy_artifact_count         BIGINT NOT NULL DEFAULT 0
...
CREATE TABLE IF NOT EXISTS jobs_runner_volume_keys (
  ...
  enrollment_epoch       BIGINT NOT NULL CHECK(enrollment_epoch >= 1),
...
CREATE TABLE IF NOT EXISTS jobs_runner_volume_storage_attestations (
  ...
  attestation_generation                 BIGINT NOT NULL CHECK(attestation_generation >= 1),
  fleet_attestation_generation           BIGINT NOT NULL UNIQUE
  ...
  predecessor_attestation_generation     BIGINT NOT NULL
  ...
  required_tombstone_generation          BIGINT NOT NULL
    CHECK(required_tombstone_generation >= 0),
  reconciled_tombstone_generation        BIGINT NOT NULL
    CHECK(reconciled_tombstone_generation >= 0),
  storage_evidence_version               BIGINT NOT NULL CHECK(storage_evidence_version = 2),
  subject_storage_layout_version         BIGINT NOT NULL
    CHECK(subject_storage_layout_version = 2),
  root_device_id                         TEXT NOT NULL,
  root_link_count                        BIGINT NOT NULL CHECK(root_link_count >= 1),
  root_entry_count                       BIGINT NOT NULL CHECK(root_entry_count >= 0),
  root_file_bytes                        TEXT NOT NULL,
  root_sha256                            TEXT NOT NULL CHECK(length(root_sha256) = 64),
  subject_storage_subject_count          BIGINT NOT NULL
  ...
  subject_storage_scope_count            BIGINT NOT NULL
    CHECK(subject_storage_scope_count >= 0),
  subject_storage_complete_root_entry_count BIGINT NOT NULL
  ...
  locator_count                          BIGINT NOT NULL CHECK(locator_count >= 0),
  resident_locator_count                 BIGINT NOT NULL CHECK(resident_locator_count >= 0),
  locator_set_sha256                     TEXT NOT NULL CHECK(length(locator_set_sha256) = 64),
  legacy_inventory_version               BIGINT NOT NULL CHECK(legacy_inventory_version = 1),
  legacy_artifact_count                  BIGINT NOT NULL CHECK(legacy_artifact_count = 0),
  ...
  unclassified_root_count                BIGINT NOT NULL CHECK(unclassified_root_count = 0),
...
CREATE OR REPLACE FUNCTION reject_jobs_runner_volume_storage_attestation_mutation()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'runner volume storage attestation is append-only';
END;
$$;

DROP TRIGGER IF EXISTS trg_jobs_runner_volume_storage_attestations_no_update
  ON jobs_runner_volume_storage_attestations;
CREATE TRIGGER trg_jobs_runner_volume_storage_attestations_no_update
BEFORE UPDATE ON jobs_runner_volume_storage_attestations
FOR EACH ROW EXECUTE FUNCTION reject_jobs_runner_volume_storage_attestation_mutation();

DROP TRIGGER IF EXISTS trg_jobs_runner_volume_storage_attestations_no_delete
  ON jobs_runner_volume_storage_attestations;
CREATE TRIGGER trg_jobs_runner_volume_storage_attestations_no_delete
...
CREATE TABLE IF NOT EXISTS jobs_execution_lease_volume_bindings (
  ...
  volume_id               TEXT NOT NULL,
  volume_epoch            BIGINT NOT NULL CHECK(volume_epoch >= 1),
  process_instance_id     TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS jobs_runner_volume_residencies (
  ...
  first_recorded_at_ms  BIGINT NOT NULL CHECK(first_recorded_at_ms >= 0),
  last_recorded_at_ms   BIGINT NOT NULL CHECK(last_recorded_at_ms >= 0),
...
CREATE TABLE IF NOT EXISTS jobs_runner_volume_destructions (
  ...
  recorded_at_ms             BIGINT NOT NULL CHECK(recorded_at_ms >= occurred_at_ms),
...
CREATE TABLE IF NOT EXISTS jobs_runner_purge_requests (
  ...
  purge_subject              TEXT NOT NULL CHECK(length(purge_subject) = 43),
  purge_generation           BIGINT NOT NULL UNIQUE CHECK(purge_generation >= 1),
  legacy_inventory_generation BIGINT NOT NULL CHECK(legacy_inventory_generation >= 1),
  ...
  legacy_unresolved_count    BIGINT NOT NULL DEFAULT 0
    CHECK(legacy_unresolved_count >= 0),
  required_target_count      BIGINT NOT NULL DEFAULT 0
    CHECK(required_target_count >= 0),
  resolved_target_count      BIGINT NOT NULL DEFAULT 0
...
CREATE TABLE IF NOT EXISTS jobs_runner_purge_targets (
  ...
  ack_inventory_before_count BIGINT,
  ack_inventory_before_sha256 TEXT,
  ack_inventory_after_count  BIGINT,
  ack_inventory_after_sha256 TEXT,
  ack_removed_entry_count BIGINT,
  ack_runner_build_id     TEXT,
  ack_completed_at_ms     BIGINT,
  created_at_ms           BIGINT NOT NULL CHECK(created_at_ms >= 0),
  updated_at_ms            BIGINT NOT NULL CHECK(updated_at_ms >= 0),
...
CREATE TABLE IF NOT EXISTS jobs_runner_purge_tombstones (
  ...
  purge_generation       BIGINT NOT NULL CHECK(purge_generation >= 1),
  tombstone_generation   BIGINT NOT NULL UNIQUE CHECK(tombstone_generation >= 1),
  target_set_sha256      TEXT NOT NULL CHECK(length(target_set_sha256) = 64),
  required_target_count  BIGINT NOT NULL CHECK(required_target_count >= 0),
