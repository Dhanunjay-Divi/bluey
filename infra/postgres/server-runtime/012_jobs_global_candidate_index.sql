...
-- Bluey Jobs global candidate-feed staging. Rows here are leads only and do
-- not satisfy original-source verification for preparation or submission.
...
CREATE TABLE IF NOT EXISTS jobs_global_discovery_sources (
  ...
  run_interval_ms       BIGINT NOT NULL DEFAULT 14400000,
...
CREATE TABLE IF NOT EXISTS jobs_global_ingestion_runs (
  ...
  received_batches      BIGINT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS jobs_global_materialization_state (
  ...
  global_updated_at_ms    BIGINT NOT NULL DEFAULT 0,
  profile_revision_at_ms  BIGINT NOT NULL DEFAULT 0,
