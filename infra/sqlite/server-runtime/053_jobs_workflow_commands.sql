...
CREATE TABLE IF NOT EXISTS jobs_workflow_commands (
  ...
  idempotency_key_hmac_sha256     TEXT NOT NULL
    CHECK(length(idempotency_key_hmac_sha256) = 64
      ...
      AND idempotency_key_hmac_sha256 NOT GLOB '*[^0-9a-f]*'),
  ...
  command_json                    TEXT NOT NULL
    CHECK(length(command_json) BETWEEN 15 AND 16777216
      AND substr(command_json, 1, 14) = 'bluey-jobs:v1:'),
  ...
  CHECK(
    ...
    OR (command_kind = 'resume' AND intervention_id IS NOT NULL
      ...
      AND intervention_id NOT GLOB '*[^A-Za-z0-9_-]*')
  ),
  ...
  CHECK(
    (state = 'accepted' AND temporal_run_id IS NOT NULL
      ...
      AND temporal_run_id NOT GLOB '*[^A-Za-z0-9_-]*'
...
CREATE TABLE IF NOT EXISTS jobs_workflow_executions (
  ...
  first_execution_run_id           TEXT NOT NULL
    CHECK(length(first_execution_run_id) BETWEEN 20 AND 128
      AND first_execution_run_id NOT GLOB '*[^A-Za-z0-9_-]*'),
  ...
  deletion_target_generation       INTEGER
  ...
  cleanup_lease_expires_at_ms      INTEGER,
  terminal_at_ms                   INTEGER,
  termination_requested_at_ms      INTEGER,
  terminated_at_ms                 INTEGER,
  history_deleted_at_ms            INTEGER,
  ...
  CHECK(
    ...
    OR (deletion_target_generation IS NOT NULL
      ...
      AND deletion_target_hmac_sha256 NOT GLOB '*[^0-9a-f]*')
  ),
  ...
  CHECK(
    ...
    OR (cleanup_lease_owner IS NOT NULL AND length(cleanup_lease_owner) BETWEEN 1 AND 128
      ...
      AND cleanup_lease_token_sha256 NOT GLOB '*[^0-9a-f]*'
...
-- It can be frozen while the general account-deletion fence is intentionally
-- delayed for irreversible side-effect reconciliation.
CREATE TABLE IF NOT EXISTS jobs_workflow_cleanup_generations (
  ...
  legacy_reconciled                INTEGER NOT NULL DEFAULT 0 CHECK(legacy_reconciled = 0),
  legacy_unresolved_count          INTEGER NOT NULL DEFAULT 1
  ...
  CHECK(state <> 'complete' OR (
    completed_at_ms IS NOT NULL AND legacy_reconciled = 1 AND legacy_unresolved_count = 0
  )),
  CHECK(updated_at_ms >= created_at_ms)
);
...
CREATE TABLE IF NOT EXISTS jobs_workflow_cleanup_targets (
  ...
  start_request_id                 TEXT NOT NULL
    CHECK(length(start_request_id) BETWEEN 20 AND 128
      AND start_request_id NOT GLOB '*[^A-Za-z0-9_-]*'),
  start_payload_hmac_sha256        TEXT NOT NULL
    CHECK(length(start_payload_hmac_sha256) = 64
      ...
      AND start_payload_hmac_sha256 NOT GLOB '*[^0-9a-f]*'),
...
CREATE TABLE IF NOT EXISTS jobs_workflow_execution_cleanup_observations (
  ...
  observed_execution_run_id        TEXT
    CHECK(observed_execution_run_id IS NULL OR (
      ...
      AND observed_execution_run_id NOT GLOB '*[^A-Za-z0-9_-]*')),
  ...
  evidence_hmac_sha256             TEXT NOT NULL
    CHECK(length(evidence_hmac_sha256) = 64
      ...
      AND evidence_hmac_sha256 NOT GLOB '*[^0-9a-f]*'),
  recorded_at_ms                   INTEGER NOT NULL
    CHECK(recorded_at_ms BETWEEN 0 AND 9007199254740991),
...
CREATE TABLE IF NOT EXISTS jobs_workflow_intervention_preparations (
  ...
  intervention_json               TEXT NOT NULL
    CHECK(length(intervention_json) BETWEEN 15 AND 16777216
      AND substr(intervention_json, 1, 14) = 'bluey-jobs:v1:'),
...
CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_command_resume_authority_insert
BEFORE INSERT ON jobs_workflow_commands
WHEN NEW.command_kind = 'resume' AND NOT EXISTS (
...
BEGIN
  SELECT RAISE(ABORT, 'workflow resume has no start authority');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_command_authority_immutable
BEFORE UPDATE OF
...
BEGIN
  SELECT RAISE(ABORT, 'workflow command authority is immutable');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_command_active_attempt
...
WHEN (NEW.active_attempt_id IS NOT NULL AND NOT EXISTS (
...
)) OR (NEW.state = 'delivering' AND NOT EXISTS (
  SELECT 1 FROM jobs_workflow_command_attempt_events AS event
   WHERE event.attempt_id = NEW.active_attempt_id
     ...
     AND event.command_id = NEW.id
     AND event.fence = NEW.fence
     AND event.event_kind = 'request_started'
)) OR (NEW.state = 'delivery_unknown' AND NOT EXISTS (
  SELECT 1 FROM jobs_workflow_command_attempt_events AS event
   WHERE event.attempt_id = NEW.active_attempt_id
     AND event.account_id = NEW.account_id
     AND event.command_id = NEW.id
     AND event.fence = NEW.fence
     AND event.event_kind = 'delivery_unknown'
...
)) OR (NEW.state = 'identity_conflict' AND NOT EXISTS (
  SELECT 1 FROM jobs_workflow_command_attempt_events AS event
   WHERE event.attempt_id = NEW.active_attempt_id
     AND event.account_id = NEW.account_id
     AND event.command_id = NEW.id
     AND event.fence = NEW.fence
     AND event.event_kind = 'identity_conflict'
...
BEGIN
  SELECT RAISE(ABORT, 'invalid workflow command attempt authority');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_attempts_no_update
BEFORE UPDATE ON jobs_workflow_command_attempts
BEGIN
  SELECT RAISE(ABORT, 'workflow command attempt is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_attempt_events_no_update
BEFORE UPDATE ON jobs_workflow_command_attempt_events
BEGIN
  SELECT RAISE(ABORT, 'workflow command attempt event is immutable');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_execution_binding_immutable
BEFORE UPDATE OF account_id, workflow_id, start_command_id, first_execution_run_id, created_at_ms
ON jobs_workflow_executions
BEGIN
  SELECT RAISE(ABORT, 'workflow execution identity is immutable');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_cleanup_generation_immutable
...
BEGIN
  SELECT RAISE(ABORT, 'workflow cleanup generation is immutable');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_cleanup_observation_no_update
BEFORE UPDATE ON jobs_workflow_execution_cleanup_observations
BEGIN
  SELECT RAISE(ABORT, 'workflow cleanup observation is immutable');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_cleanup_target_identity_immutable
...
ON jobs_workflow_cleanup_targets
BEGIN
  SELECT RAISE(ABORT, 'workflow cleanup target identity is immutable');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_cleanup_target_transition_evidence
BEFORE UPDATE OF first_execution_run_id, target_state, absence_proved_at_ms
ON jobs_workflow_cleanup_targets
WHEN
  (OLD.target_state IN ('absence_proved', 'identity_conflict')
    AND NEW.target_state <> OLD.target_state)
  OR (OLD.first_execution_run_id IS NOT NULL
    AND NEW.first_execution_run_id IS NOT OLD.first_execution_run_id)
  OR (NEW.target_state = 'identity_reconcile'
    AND OLD.target_state = 'delivery_drain' AND NOT EXISTS (
  ...
  OR (NEW.target_state = 'cleanup_required'
    AND OLD.target_state IN ('delivery_drain', 'identity_reconcile') AND NOT (
      NEW.first_execution_run_id IS NOT NULL AND (
        EXISTS (
  ...
  OR (NEW.target_state = 'identity_conflict'
    AND OLD.target_state <> 'identity_conflict' AND NOT EXISTS (
      SELECT 1 FROM jobs_workflow_execution_cleanup_observations observation
       WHERE observation.account_id = NEW.account_id
         AND observation.workflow_id = NEW.workflow_id
         AND observation.generation = NEW.generation
  ...
  OR (NEW.target_state = 'termination_pending'
    AND OLD.target_state <> 'termination_pending' AND NOT EXISTS (
      SELECT 1 FROM jobs_workflow_execution_cleanup_observations observation
       WHERE observation.account_id = NEW.account_id
         AND observation.workflow_id = NEW.workflow_id
         AND observation.generation = NEW.generation
  ...
  OR (NEW.target_state = 'history_delete_pending'
    AND OLD.target_state <> 'history_delete_pending' AND NOT (
      EXISTS (
        SELECT 1 FROM jobs_workflow_execution_cleanup_observations observation
         WHERE observation.account_id = NEW.account_id
           AND observation.workflow_id = NEW.workflow_id
  ...
  OR (NEW.target_state = 'absence_proved'
    AND OLD.target_state <> 'absence_proved' AND NOT (
      ...
      AND EXISTS (SELECT 1 FROM jobs_workflow_execution_cleanup_observations observation
        WHERE observation.account_id = NEW.account_id
          ...
          AND observation.observed_execution_run_id IS NEW.first_execution_run_id)
    ))
BEGIN
  SELECT RAISE(ABORT, 'workflow cleanup target transition lacks exact evidence');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_cleanup_generation_complete_guard
...
WHEN NEW.state = 'complete' AND OLD.state <> 'complete' AND (
  NEW.completed_at_ms IS NULL OR NEW.legacy_reconciled <> 1
  OR NEW.legacy_unresolved_count <> 0
  OR (SELECT COUNT(*) FROM jobs_workflow_cleanup_targets target
...
BEGIN
  SELECT RAISE(ABORT, 'workflow cleanup generation is not exactly complete');
END;

CREATE TRIGGER IF NOT EXISTS trg_jobs_workflow_cleanup_transition_evidence
BEFORE UPDATE OF lifecycle_state, cleanup_state, deletion_target_generation,
  deletion_target_hmac_sha256
ON jobs_workflow_executions
WHEN
  (NEW.deletion_target_generation IS NOT NULL AND NOT EXISTS (
  ...
  OR (NEW.lifecycle_state = 'termination_requested'
  ...
  OR (NEW.lifecycle_state = 'terminated'
    AND OLD.lifecycle_state <> 'terminated' AND NOT EXISTS (
      SELECT 1 FROM jobs_workflow_execution_cleanup_observations observation
       WHERE observation.account_id = NEW.account_id
         AND observation.workflow_id = NEW.workflow_id
         AND observation.generation = NEW.deletion_target_generation
  ...
  OR (NEW.cleanup_state = 'history_deleted'
    AND OLD.cleanup_state <> 'history_deleted' AND NOT EXISTS (
      SELECT 1 FROM jobs_workflow_execution_cleanup_observations observation
       WHERE observation.account_id = NEW.account_id
         AND observation.workflow_id = NEW.workflow_id
         AND observation.generation = NEW.deletion_target_generation
  ...
  OR (NEW.cleanup_state = 'absence_proved'
    AND OLD.cleanup_state <> 'absence_proved' AND NOT (
...
BEGIN
  SELECT RAISE(ABORT, 'workflow cleanup transition lacks exact evidence');
END;
...
BEGIN
  SELECT RAISE(ABORT, 'workflow external state is unresolved');
END;
