...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_trust_policies (
  ...
  maximum_clock_skew_ms          INTEGER NOT NULL CHECK(maximum_clock_skew_ms BETWEEN 0 AND 300000),
  maximum_manifest_size_bytes    INTEGER NOT NULL CHECK(maximum_manifest_size_bytes BETWEEN 1024 AND 65536),
  maximum_target_count           INTEGER NOT NULL CHECK(maximum_target_count BETWEEN 1 AND 32),
  maximum_observation_count      INTEGER NOT NULL CHECK(maximum_observation_count BETWEEN 1 AND 64),
  maximum_evidence_object_count  INTEGER NOT NULL CHECK(maximum_evidence_object_count BETWEEN 1 AND 64),
  ...
  recorded_at_ms                  INTEGER NOT NULL CHECK(recorded_at_ms >= valid_from_ms),
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_trust_head (
  ...
  current_trust_generation        INTEGER NOT NULL UNIQUE
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_evidence (
  ...
  issued_at_ms                    INTEGER NOT NULL CHECK(issued_at_ms >= captured_at_ms),
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_layout_observations (
  ...
  issued_at_ms                    INTEGER NOT NULL CHECK(issued_at_ms >= observed_at_ms),
  expires_at_ms                   INTEGER NOT NULL CHECK(expires_at_ms > issued_at_ms),
  recorded_by                     TEXT NOT NULL CHECK(length(recorded_by) > 0),
  recorded_at_ms                  INTEGER NOT NULL CHECK(recorded_at_ms >= issued_at_ms),
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_manifests (
  ...
  layout_contract_version         INTEGER NOT NULL
  ...
  layout_observation_count        INTEGER NOT NULL CHECK(layout_observation_count BETWEEN 1 AND 64),
  check_result_count              INTEGER NOT NULL CHECK(check_result_count BETWEEN 1 AND 1536),
  hard_filter_violations          INTEGER NOT NULL CHECK(hard_filter_violations = 0),
  unsupported_factual_claims      INTEGER NOT NULL CHECK(unsupported_factual_claims = 0),
  duplicate_submit_activations    INTEGER NOT NULL CHECK(duplicate_submit_activations = 0),
  false_submitted_states          INTEGER NOT NULL CHECK(false_submitted_states = 0),
  incomplete_receipts             INTEGER NOT NULL CHECK(incomplete_receipts = 0),
  pii_bearing_observations        INTEGER NOT NULL CHECK(pii_bearing_observations = 0),
  evidence_count                  INTEGER NOT NULL CHECK(evidence_count BETWEEN 1 AND 64),
  runtime_target_count            INTEGER NOT NULL CHECK(runtime_target_count BETWEEN 1 AND 32),
  ...
  CHECK(
    ...
    OR (provider = 'greenhouse' AND adapter_version = '2026.07.1-beta.1'
      AND target_key GLOB 'greenhouse:*:*'
    ...
    OR (provider = 'lever' AND adapter_version = '2026.07.0-beta.1'
      ...
      AND ((target_key GLOB 'lever:jobs.lever.co:*:*'
          AND allowed_provider_hosts_json = '["jobs.lever.co"]')
        OR (target_key GLOB 'lever:jobs.eu.lever.co:*:*'
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_manifest_check_results (
  ...
  ordinal                         INTEGER NOT NULL CHECK(ordinal BETWEEN 0 AND 1535),
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_activations (
  ...
  canary_max_submissions          INTEGER NOT NULL
    CHECK(canary_max_submissions BETWEEN 0 AND 10000),
  canary_account_cap              INTEGER NOT NULL CHECK(canary_account_cap BETWEEN 0 AND 10000),
  canary_concurrency_cap          INTEGER NOT NULL CHECK(canary_concurrency_cap BETWEEN 0 AND 1000),
  canary_daily_side_effect_cap    INTEGER NOT NULL
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_quarantine_heads (
  ...
  current_command_sequence        INTEGER NOT NULL
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_circuit_events (
  ...
  window_ended_at_ms              INTEGER NOT NULL CHECK(window_ended_at_ms >= window_started_at_ms),
  failure_count                   INTEGER NOT NULL CHECK(failure_count >= 0),
  sample_count                    INTEGER NOT NULL CHECK(sample_count >= failure_count),
  threshold_count                 INTEGER NOT NULL CHECK(threshold_count >= 0),
  authority_ref                   TEXT NOT NULL CHECK(length(authority_ref) > 0),
  event_at_ms                     INTEGER NOT NULL CHECK(event_at_ms >= window_ended_at_ms),
  recorded_by                     TEXT NOT NULL CHECK(length(recorded_by) > 0),
  recorded_at_ms                  INTEGER NOT NULL CHECK(recorded_at_ms >= event_at_ms),
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_circuit_heads (
  ...
  head_revision                   INTEGER NOT NULL CHECK(head_revision BETWEEN 1 AND 9007199254740991),
...
CREATE TABLE IF NOT EXISTS jobs_ats_certification_runtime_layout_quarantine_evidence (
  ...
  observed_layout_contract_version INTEGER NOT NULL
...
CREATE TABLE IF NOT EXISTS jobs_application_ats_certification_bindings (
  ...
  auto_authorization_revision     INTEGER NOT NULL CHECK(auto_authorization_revision > 0),
...
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_trust_policies_no_update
BEFORE UPDATE ON jobs_ats_certification_trust_policies BEGIN
  SELECT RAISE(ABORT, 'ATS certification trust policy is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_trust_policies_no_delete
BEFORE DELETE ON jobs_ats_certification_trust_policies BEGIN
  SELECT RAISE(ABORT, 'ATS certification trust policy is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_trust_keys_no_update
BEFORE UPDATE ON jobs_ats_certification_trust_keys BEGIN
  SELECT RAISE(ABORT, 'ATS certification trust key is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_trust_keys_no_delete
BEFORE DELETE ON jobs_ats_certification_trust_keys BEGIN
  SELECT RAISE(ABORT, 'ATS certification trust key is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_trust_head_monotonic
...
WHEN NEW.head_revision <> OLD.head_revision + 1
  ...
  OR NEW.root_trust_anchor_sha256 <> OLD.root_trust_anchor_sha256
BEGIN
  SELECT RAISE(ABORT, 'ATS certification trust head must advance monotonically');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_trust_head_no_delete
BEFORE DELETE ON jobs_ats_certification_trust_head BEGIN
  SELECT RAISE(ABORT, 'ATS certification trust head cannot be deleted');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_evidence_no_update
BEFORE UPDATE ON jobs_ats_certification_evidence BEGIN
  SELECT RAISE(ABORT, 'ATS certification evidence is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_evidence_no_delete
BEFORE DELETE ON jobs_ats_certification_evidence BEGIN
  SELECT RAISE(ABORT, 'ATS certification evidence is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_layout_observations_no_update
BEFORE UPDATE ON jobs_ats_certification_layout_observations BEGIN
  SELECT RAISE(ABORT, 'ATS certification layout observation is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_layout_observations_no_delete
BEFORE DELETE ON jobs_ats_certification_layout_observations BEGIN
  SELECT RAISE(ABORT, 'ATS certification layout observation is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_manifests_no_update
BEFORE UPDATE ON jobs_ats_certification_manifests BEGIN
  SELECT RAISE(ABORT, 'ATS certification manifest is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_manifests_no_delete
BEFORE DELETE ON jobs_ats_certification_manifests BEGIN
  SELECT RAISE(ABORT, 'ATS certification manifest is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_manifest_evidence_no_update
BEFORE UPDATE ON jobs_ats_certification_manifest_evidence BEGIN
  SELECT RAISE(ABORT, 'ATS certification evidence binding is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_manifest_layouts_no_update
BEFORE UPDATE ON jobs_ats_certification_manifest_layouts BEGIN
  SELECT RAISE(ABORT, 'ATS certification manifest layout binding is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_manifest_layouts_no_delete
BEFORE DELETE ON jobs_ats_certification_manifest_layouts BEGIN
  SELECT RAISE(ABORT, 'ATS certification manifest layout binding is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_manifest_check_results_no_update
BEFORE UPDATE ON jobs_ats_certification_manifest_check_results BEGIN
  SELECT RAISE(ABORT, 'ATS certification check result is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_manifest_check_results_no_delete
BEFORE DELETE ON jobs_ats_certification_manifest_check_results BEGIN
  SELECT RAISE(ABORT, 'ATS certification check result is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_manifest_evidence_no_delete
BEFORE DELETE ON jobs_ats_certification_manifest_evidence BEGIN
  SELECT RAISE(ABORT, 'ATS certification evidence binding is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_runtime_targets_no_update
BEFORE UPDATE ON jobs_ats_certification_runtime_targets BEGIN
  SELECT RAISE(ABORT, 'ATS certification runtime target is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_runtime_targets_no_delete
BEFORE DELETE ON jobs_ats_certification_runtime_targets BEGIN
  SELECT RAISE(ABORT, 'ATS certification runtime target is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_activations_no_update
BEFORE UPDATE ON jobs_ats_certification_activations BEGIN
  SELECT RAISE(ABORT, 'ATS certification activation is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_activations_no_delete
BEFORE DELETE ON jobs_ats_certification_activations BEGIN
  SELECT RAISE(ABORT, 'ATS certification activation is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_revocations_chain
...
BEGIN
  SELECT RAISE(ABORT, 'ATS certification revocation chain is invalid');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_revocations_no_update
BEFORE UPDATE ON jobs_ats_certification_revocations BEGIN
  SELECT RAISE(ABORT, 'ATS certification revocation is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_revocations_no_delete
BEFORE DELETE ON jobs_ats_certification_revocations BEGIN
  SELECT RAISE(ABORT, 'ATS certification revocation is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_head_transitions_no_update
BEFORE UPDATE ON jobs_ats_certification_head_transitions BEGIN
  SELECT RAISE(ABORT, 'ATS certification head transition is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_head_transitions_no_delete
BEFORE DELETE ON jobs_ats_certification_head_transitions BEGIN
  SELECT RAISE(ABORT, 'ATS certification head transition is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_quarantine_commands_no_update
BEFORE UPDATE ON jobs_ats_certification_quarantine_commands BEGIN
  SELECT RAISE(ABORT, 'ATS certification quarantine command is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_quarantine_commands_no_delete
BEFORE DELETE ON jobs_ats_certification_quarantine_commands BEGIN
  SELECT RAISE(ABORT, 'ATS certification quarantine command is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_heads_monotonic
...
BEGIN
  SELECT RAISE(ABORT, 'ATS certification head must advance monotonically');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_heads_no_delete
BEFORE DELETE ON jobs_ats_certification_heads BEGIN
  SELECT RAISE(ABORT, 'ATS certification head cannot be deleted');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_quarantine_heads_monotonic
BEFORE UPDATE ON jobs_ats_certification_quarantine_heads
WHEN NEW.head_revision <> OLD.head_revision + 1
  OR NEW.current_command_sequence <= OLD.current_command_sequence
  OR NEW.current_command_sha256 = OLD.current_command_sha256
BEGIN
  SELECT RAISE(ABORT, 'ATS certification quarantine head must advance monotonically');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_quarantine_heads_no_delete
BEFORE DELETE ON jobs_ats_certification_quarantine_heads BEGIN
  SELECT RAISE(ABORT, 'ATS certification quarantine head cannot be deleted');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_circuit_events_no_update
BEFORE UPDATE ON jobs_ats_certification_circuit_events BEGIN
  SELECT RAISE(ABORT, 'ATS certification circuit event is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_circuit_events_no_delete
BEFORE DELETE ON jobs_ats_certification_circuit_events BEGIN
  SELECT RAISE(ABORT, 'ATS certification circuit event is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_runtime_layout_quarantine_no_update
BEFORE UPDATE ON jobs_ats_certification_runtime_layout_quarantine_evidence BEGIN
  SELECT RAISE(ABORT, 'ATS runtime layout quarantine evidence is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_runtime_layout_quarantine_no_delete
BEFORE DELETE ON jobs_ats_certification_runtime_layout_quarantine_evidence BEGIN
  SELECT RAISE(ABORT, 'ATS runtime layout quarantine evidence is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_circuit_heads_monotonic
BEFORE UPDATE ON jobs_ats_certification_circuit_heads
WHEN NEW.head_revision <> OLD.head_revision + 1 OR NEW.current_event_id = OLD.current_event_id
BEGIN
  SELECT RAISE(ABORT, 'ATS certification circuit head must advance monotonically');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_circuit_heads_no_delete
BEFORE DELETE ON jobs_ats_certification_circuit_heads BEGIN
  SELECT RAISE(ABORT, 'ATS certification circuit head cannot be deleted');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_canary_allowlists_no_update
BEFORE UPDATE ON jobs_ats_certification_canary_allowlists BEGIN
  SELECT RAISE(ABORT, 'ATS certification canary allowlist is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_canary_allowlists_no_delete
BEFORE DELETE ON jobs_ats_certification_canary_allowlists BEGIN
  SELECT RAISE(ABORT, 'ATS certification canary allowlist is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_canary_allowlist_members_no_update
BEFORE UPDATE ON jobs_ats_certification_canary_allowlist_members BEGIN
  SELECT RAISE(ABORT, 'ATS certification canary allowlist member is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_canary_allowlist_members_no_delete
BEFORE DELETE ON jobs_ats_certification_canary_allowlist_members BEGIN
  SELECT RAISE(ABORT, 'ATS certification canary allowlist member is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_canary_allowlist_revocations_no_update
BEFORE UPDATE ON jobs_ats_certification_canary_allowlist_revocations BEGIN
  SELECT RAISE(ABORT, 'ATS certification canary allowlist revocation is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_canary_allowlist_revocations_no_delete
BEFORE DELETE ON jobs_ats_certification_canary_allowlist_revocations BEGIN
  SELECT RAISE(ABORT, 'ATS certification canary allowlist revocation is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_application_ats_certification_bindings_transition
BEFORE UPDATE ON jobs_application_ats_certification_bindings
WHEN OLD.phase <> 'preflight'
  ...
  OR NEW.browser_release_manifest_sha256 IS NOT OLD.browser_release_manifest_sha256
  OR NEW.browser_artifact_sha256 IS NOT OLD.browser_artifact_sha256
  OR NEW.browser_build_descriptor_sha256 IS NOT OLD.browser_build_descriptor_sha256
  OR NEW.runner_build_id IS NOT OLD.runner_build_id
  OR NEW.runner_image_sha256 IS NOT OLD.runner_image_sha256
  ...
  OR NEW.fence <> 1
BEGIN
  SELECT RAISE(ABORT, 'ATS application certification binding transition is invalid');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_application_ats_certification_bindings_no_delete
BEFORE DELETE ON jobs_application_ats_certification_bindings BEGIN
  SELECT RAISE(ABORT, 'ATS application certification binding cannot be deleted');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_canary_reservations_no_update
BEFORE UPDATE ON jobs_ats_certification_canary_reservations BEGIN
  SELECT RAISE(ABORT, 'ATS certification canary reservation is immutable');
END;
CREATE TRIGGER IF NOT EXISTS trg_jobs_ats_certification_canary_reservations_no_delete
BEFORE DELETE ON jobs_ats_certification_canary_reservations BEGIN
  SELECT RAISE(ABORT, 'ATS certification canary reservation is immutable');
END;
