...
fn postgres_local_run_claim_and_submit_recheck_current_authority() {
    ...
    assert!(
        local_run_submit_authorized(&pool, &live.run_id, &live.ticket_hash)
            .expect("check live PostgreSQL submit authority")
    );
    ...
    assert!(
        local_run_submit_authorized(&pool, &live.run_id, &live.ticket_hash)
            .expect("recheck restored PostgreSQL submit authority")
    );
