...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/jobs/applications/:application_id/bluey-handoff",
            post(super::jobs_handoff::issue).route_layer(DefaultBodyLimit::max(1_024)),
        )
        .route(
            "/api/jobs/bluey-handoffs/redeem",
            post(super::jobs_handoff::redeem).route_layer(DefaultBodyLimit::max(1_024)),
        )
        .route(
