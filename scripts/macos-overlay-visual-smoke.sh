...
require_source "override func keyDown(with event: NSEvent)"
require_source "event.keyCode == 36 || event.keyCode == 76"
require_source "event.modifierFlags.contains(.shift)"
require_source "insertNewline(nil)"
require_source "onSubmit?()"
require_source "window?.makeFirstResponder(composer)"
...
require_source "fullSizeButton.toolTip = windowFullSize ? \"Restore compact Bluey\" : \"Fill this screen\""
require_source "fullSizeButton.action = #selector(fullSizeClicked)"
require_source "Move-anywhere on: controls click normally, and blank Bluey space drags the panel."
require_source "let targetFrame = preWindowFullSizeFrame"
