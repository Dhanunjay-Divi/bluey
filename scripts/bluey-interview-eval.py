...
from bluey_eval.lru_contracts import (  # noqa: E402
    lru_code_semantic_issues,
    python_source_from_code_artifact,
)
...
CASES: Tuple[EvalCase, ...] = (
    ...
    EvalCase("Q02", "behavioral", "sde", "Walk me through the most technically challenging backend project you built.", "otter_interview_style", "sde_project", speakable=True, expected_outcome="needs_user_input", required_groups=(g("payment", "monitoring", "order matching"), g("scale", "100k", "50m", "10k"))),
    EvalCase("Q03", "followup", "sde", "Why did you choose that architecture, and what tradeoff did you accept?", "otter_interview_style", "sde_project", speakable=True, expect_followup_context=True, expected_outcome="needs_user_input", required_groups=(g("tradeoff", "because", "chose"),)),
    EvalCase("Q04", "behavioral", "sde", "Tell me about a production issue you owned from detection through rollout.", "behavioral_doc", speakable=True, expected_outcome="needs_user_input", required_groups=(g("monitor", "incident", "production"), g("verify", "test", "rollout", "metric"))),
    ...
    EvalCase("Q12", "behavioral", "sde", "Tell me about a time you disagreed with a product or engineering decision and how you handled it.", "behavioral_doc", speakable=True, expected_outcome="needs_user_input", required_groups=(g("disagree", "concern", "tradeoff"), g("data", "evidence", "experiment"), g("align", "decision", "commit"))),
    ...
    EvalCase("Q14", "behavioral", "de", "Walk me through a data pipeline you built that had meaningful scale and business impact.", "otter_interview_style", "de_pipeline", speakable=True, expected_outcome="needs_user_input", required_groups=(g("spark", "kafka", "glue"), g("terabyte", "scale"), g("capital one", "fidelity"))),
    EvalCase("Q15", "followup", "de", "How did you prove the data was correct before downstream teams trusted it?", "otter_interview_style", "de_pipeline", speakable=True, expect_followup_context=True, expected_outcome="needs_user_input", required_groups=(g("reconcil", "row count", "control total"), g("quality", "validation", "schema"), g("monitor", "alert"))),
    ...
    EvalCase("Q25", "behavioral", "ds", "Walk me through the secure RAG system you built and the decision you personally owned.", "otter_interview_style", "rag_project", speakable=True, expected_outcome="needs_user_input", required_groups=(g("2m", "document"), g("98%", "precision"), g("secure", "access"))),
    EvalCase("Q26", "followup", "ds", "Where could that RAG system hallucinate, and what did you put in place to catch it?", "otter_interview_style", "rag_project", speakable=True, expect_followup_context=True, expected_outcome="needs_user_input", required_groups=(g("retriev", "ground"), g("citation", "source"), g("eval", "threshold", "fallback"))),
    ...
    if case.expect_code:
        if attempt.artifact_type != "code":
            ...
            if case.id in ("Q08", "Q09"):
                issues.extend(
                    lru_code_semantic_issues(case.id, attempt.artifact_body or "")
                )
