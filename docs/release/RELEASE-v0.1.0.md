...
| Item | Source | Status |
|---|---|---|
| Reset overlay UI state on cancel/error (not just submit) | Codex R12 review nit | Done in R13 Codex pass |
| `generate_session_token` returns `Result` instead of `expect()`-panic | Codex R12 review nit | Done in R13 Codex pass |
...
| File | Notes |
|---|---|
| `bluey-0.1.0-darwin-arm64.tar.gz` | the shipped tarball |
| `SHA256SUMS.txt` | release-level checksums generated by GitHub Actions |
| `sha256-manifest.json` | machine-readable checksum manifest for formula updates |
