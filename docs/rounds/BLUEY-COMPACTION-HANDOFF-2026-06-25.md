...
- Round 319 is complete and deployed for Tone typing/focus visibility:
  ...
  - cursor behavior is unchanged and still stays arrow-style inside Bluey controls
  - Windows parity check: the current Windows overlay does not have the same Tone editor path, and its syntax check passed
  - workspace desktop version bumped to `0.1.58`
  - verification passed:
...
## Latest Round 351: Click-Through Content Scroll Policy
...
- `docs/rounds/ROUND-351-CLICKTHROUGH-CONTENT-SCROLL-POLICY.md`

Trigger:

- The owner asked for a better click-through scroll model:
  - scroll over question/answer content should scroll Bluey
  - scroll over blank black/empty panel area should pass through to the app behind Bluey
  - a visible Bluey scrollbar/scroller should remain an intentional scroll target

Product decision:

- In click-through mode, Bluey captures feed scrolling only over real rendered card bubbles or the visible feed scroller.
- Blank feed glass/background no longer acts like an invisible Bluey scroll target.
- In normal interactive mode, the full feed region can still scroll Bluey because Bluey is intentionally active.

What changed:

- Added `FeedView.shouldCapturePassThroughScroll(at:)`.
- It allows click-through feed scroll only over:
  - visible question/answer/card bubble rects, or
  - the visible vertical feed scroller.
- Updated `ExpandedPanelView.scrollWheel(with:)` pass-through feed routing.
- Updated `ExpandedPanelView.routeScrollWheelAtScreenPoint(_:event:)` global click-through routing.
- Desktop workspace version bumped to `0.1.90`.

Windows parity:

- This round changes macOS AppKit click-through scroll routing.
- Windows overlay is a separate native C implementation and does not share this Swift `FeedView` / `ExpandedPanelView` path.
- Desired Windows parity behavior is the same product rule: content or visible scroller captures scroll; blank overlay space passes through.
...
- The owner reported that attaching screen context or documents does not work while Bluey is streaming an answer.

Root cause:

- The daemon overlay event loop processed events serially.
- `AskRequested` awaited the full streamed answer, so `AttachRequested`, `AttachFilesRequested`, and `AnalyzeScreenRequested` were stuck behind the active answer.
- The macOS overlay also only staged newly arrived context during a short mutation window, so context that arrived during a stream could look saved but not be pending for the next answer.

What changed:

- Overlay answer requests now run in a background task.
- The daemon overlay event loop remains responsive while answers stream.
- A daemon-side `overlay_answer_active` guard prevents accidental second answers during an active stream.
- Trying to start a second overlay answer now shows an "Answer already running" warning, while files/screens can still be prepared.
- macOS context updates stage newly added items for the next answer when attach/screen was armed or an answer is currently streaming.
...
- Desktop release `0.1.87` is live on `https://bluey.sh/latest.json`.
- Darwin arm64 artifact:
...
- Release verification passed:
  ...
  - installer MIME checks
  - Darwin arm64 artifact SHA verification
  - unpacked `bluey` and `bluey-daemon` version checks for `0.1.87`
- Public installer smoke installed `0.1.87` locally and both installed binaries report `0.1.87`.
- `bluey on` started fresh daemon pid `4255`.

Windows parity:

- The daemon event-loop fix applies to both macOS and Windows.
- The macOS staging change is Swift-only; Windows must preserve the same behavior in its overlay path: files/screens added during an active answer should be pending for the next answer, not hidden as saved-only context.
...
- The owner pointed out that Bluey can mention line numbers and `Line notes`, but the code canvas did not visually show line numbers.
- The owner also clarified that copied code must stay clean and must not include any visual line numbers.

What changed:

- macOS code canvas now shows display-only line numbers for the `CODE`, `PATCH`, `DIFF`, or changed-block section.
- `LINE NOTES`, `COMPLEXITY`, and prose sections remain unnumbered.
- The canvas copy button still copies the raw code section through the existing clean copy path.
- Keyboard copy from selected canvas text strips visual line-number gutters before writing to the pasteboard.
- Comment tinting still works after adding the display gutter.
...
- Desktop release `0.1.85` is live on `https://bluey.sh/latest.json`.
- Darwin arm64 artifact:
...
- Release verification passed:
  ...
  - installer MIME checks
  - Darwin arm64 artifact SHA verification
  - unpacked `bluey` and `bluey-daemon` version checks for `0.1.85`
- Public installer smoke installed `0.1.85` locally and both installed binaries report `0.1.85`.
- `bluey on` started fresh daemon pid `12534`.

Windows parity:

- The Windows C overlay does not yet render the same macOS right-side code canvas artifact pane.
- No Windows code change was applicable in this round.
- When Windows gets the matching code canvas UI, visible line-number gutters must be display-only and all copy paths must strip them before clipboard writes.
...
- The owner showed `IMG_3713.HEIC` where a previously fixed coding-answer issue appeared again as smashed inline `Codecppclass...` text.
- The feed showed a very large blank vertical gap before the next question.
- A follow-up PDF request failed with `Ref: DDAC3482`.

Diagnosis:

- Installed binaries were `0.1.79`, but the active local daemon process was started at `Sat Jul 4 04:47:03 2026`, before the `0.1.79` install.
- Local logs for `DDAC3482` show the request was handled by daemon version `0.1.72` through `bluey_managed/vision`, which failed with provider `400`.
- The large blank feed gap came from the macOS feed stack being forced to fill the whole scroll viewport, allowing a malformed/short legacy answer row to absorb extra height.

What changed:

- `bluey on` and daemon start paths now detect when the installed `bluey-daemon` binary is newer than the running daemon's `started_at`.
- When that happens, the CLI gracefully shuts down the stale daemon and starts the installed daemon automatically.
- Added a unit test for the daemon-binary mtime restart decision.
- The macOS feed no longer forces the stack to fill the full scroll viewport, and card spacing/insets are tighter.
- The overlay defensively hides legacy compressed inline code blobs such as `Codecppclass...` from chat, while leaving code canvas/artifact flows intact.
