...
| Gemini | ✅ | ✅ | ✅ | ⚠️ fork (quota-blocked) | ⚠️ | **Google gate CLEARED 2026-06-19** (via OAuth re-login — NOT an API key; `GEMINI_API_KEY` is still unset, `selectedAuthType` still oauth-personal, but the re-login fixed the "no longer supported" gate). Handshake now succeeds (`session_id` returned). Caps 1-3 read fine. **BUT Cap 4/5 now blocked by the FREE-TIER DAILY QUOTA:** `bluey ask` → *"You have exhausted your daily quota on this model."* (the input/output limit the user flagged). So the gate is fixed; the answer path is quota-gated until the daily reset (or a paid key). **Cap 4 = fork/replay by design** (registry `Replay`; `--resume` is latest/index, spine sends no id). Planted MARIGOLD-3982 session ready to test once quota resets. **Cap 5:** `mcpServers.github` (env-auth) parsed. |
...
| Antigravity | ✅ | ✅ | ✅ | ⚠️ fork (quota-blocked) | ✅ | **Wired up + read/MCP VERIFIED 2026-06-19; drive quota-blocked.** CORRECTED the stale "GUI-only, no CLI" belief: Antigravity DOES ship a CLI (`agy` v1.0.10, a real Go binary), but `agy` itself has NO ACP mode (only --print/--conversation/TUI). ACP comes via the sibling **`gemini --acp`**, which Antigravity's registry row already drives through. **Code fix:** `spec_map.rs` Antigravity arm changed from `bail!("GUI IDE…")` → `gemini --acp` spec. **Caps 1-3:** reads its OWN `~/.gemini/antigravity` store — real titles + projects ("Analyzing The Bluey Architecture" `[/…/Bluey]`, etc.). **Cap 5 config:** 8 connectors parsed (github, perplexity-ask, supabase×5, tradingview). **Cap 4/5 drive:** wiring PROVEN — `bluey ask` spawned `gemini --acp` and reached the model, but hit the SAME *"exhausted your daily quota"* Gemini free-tier limit (Antigravity shares the gemini account). So the ACP path is correct; the live answer is quota-gated. Registry stays `Replay` (`agy --conversation=<uuid>` could resume natively — the exposed UUID is already the right key — but agy isn't assumed installed; opt-in later). |
