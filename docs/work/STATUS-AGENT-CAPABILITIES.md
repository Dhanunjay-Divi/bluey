...
| Cursor | ✅ | ✅ | ✅ | 🚫 resume N/A / fork (blocked) | ⚠️ | **Investigated + handshake-probed 2026-06-19.** ACP plumbing WORKS (handshake reached the agent, real JSON-RPC reply) but **account is on free tier and HIT ITS USAGE LIMIT** (`"You've hit your usage limit. Get Cursor Pro…"`) → no live ask possible until Pro/reset. Caps 1-3 read fine from the `state.vscdb` SQLite store. **Cap 4 — true resume is architecturally N/A (honest):** registry has Cursor = `Replay`; its session id is a SQLite **composer UUID** (`composerData:<uuid>`, 227 rows) — a local IDE db key, NOT an ACP session handle. Cursor exposes no `session/load`-by-composer-id path; there's even a 3rd id-space (`~/.cursor/chats/<ws>/<uuid>/`) Bluey doesn't read. So fork/replay is its correct-and-only mode (registry already right). **Cap 5:** `~/.cursor/mcp.json` has 7 servers (context7, perplexity, 2× supabase http, retellai, tradingview, …); reader parses it (JSONC-tolerant); discovery showed 5/7 ready. Live MCP unverifiable until usage limit clears. BLOCKER: Cursor Pro / usage reset (account-side, not a Bluey bug). |
...
| Gemini | ✅ | ✅ | ✅ | 🚫 resume N/A / fork (blocked) | ⚠️ | read/name/project VERIFIED 2026-06-18. **Cap 4/5 BLOCKED by a Google-SIDE gate (confirmed STILL ACTIVE 2026-06-19):** the ACP handshake reaches `session/new` then the agent returns *"This client is no longer supported for Gemini Code Assist for individuals. To continue using Gemini, please migrate to the Antigravity suite…"*. Tied to `oauth-personal` auth; only a Gemini-side fix unblocks (set `GEMINI_API_KEY` BYOT, or migrate) — NOT a code change. **Cap 4 — true resume N/A by design:** registry = `Replay`; `--resume` takes "latest"/index not uuid; spine correctly replays (never sends a resume id). (Note: filename stem is an 8-char prefix of the inner `sessionId` UUID — a real id-source discrepancy, but MOOT since resume id is never sent for Replay agents.) **Cap 5:** `~/.gemini/settings.json` `mcpServers` has `github` (env-auth); reader parses it. Verifiable once the gate clears. |
