# IMPL: Jobs Round 586 - Track Auto-submit Authority And Form Read-back
...
| File | Action | Purpose |
|------|--------|---------|
| `infra/sqlite/server-runtime/040_jobs_auto_submit_authorizations.sql` | Added | SQLite authorization revisions and active-row uniqueness |
| `infra/postgres/server-runtime/018_jobs_auto_submit_authorizations.sql` | Added | PostgreSQL authorization revisions and indexes |
...
These commands are the required release gate and must pass before merge.
...
Production dependency overrides update DOMPurify, fast-uri, PostCSS, tar, and
React Router. `npm audit --omit=dev` still reports the React Router RSC action
CSRF advisory. Jobs is a client-only Vite SPA and implements neither RSC nor
server actions, so the affected execution path is not reachable in this
deployment. The residual is documented and must be reevaluated when an upstream
release provides a compatible fix or if the portal architecture changes.

## Deviations from Plan

| Deviation | Rationale |
|-----------|-----------|
| Production flags remain disabled | Runner distribution remains intentionally disabled until adapter and recovery certification |
