...
No blocking issues found in the first-version implementation after smoke testing. The current build starts with `bluey on`, opens the native overlay, starts or reuses a meeting session, can continue or start a clean session from the overlay, accepts manual transcript input, attaches readable user-selected context files, shows attached context from the overlay, stores answer instructions, supports user-triggered screenshot capture through support flows, supports active browser page analysis from the overlay, can run native chunk audio transcription when STT credentials are configured, detects questions/action items/decisions, renders overlay cards, returns recaps, archives meetings, and shuts down cleanly. The macOS overlay is now draggable, resizable, opacity-adjustable, capture-excluded, frame-persistent, has a command-bar ask composer with model/mode selection, and has a boot animation instead of being pinned to one corner.
...
- The daemon currently uses local TCP on `127.0.0.1:57321`; this is simple for development, but we should move to Unix domain sockets on macOS/Linux and named pipes on Windows before a polished release.
- Meeting records are persisted as JSON files for the first testable build. SQLite is still the right long-term store once audio/STT and recap writes are active.
- The macOS overlay is functional and uses `NSWindow.sharingType = .none`, but it still needs visual QA against Zoom/Meet/Teams and macOS screenshot/recording flows.
...
- The answer layer can call OpenAI-compatible providers when keys are configured and falls back deterministically offline. STT can call an OpenAI-compatible transcription endpoint through native chunk capture, but true streaming partials, VAD, reconnects, and provider fallback are still pending.
...
- `bluey memory search` is local development retrieval over JSON meeting records. Production RAG should move to authenticated cloud storage and tenant-scoped vector search.
- Overlay file picker and answer-style prompts are implemented for macOS first.
...
- Harden audio capture/STT with VAD, partial streaming, reconnects, provider fallback, device settings, and Windows hardware QA.
- Add managed provider answer streaming with markdown and citations behind the existing answer request/response/event contract.
...
- Package signed desktop apps with auto-update, crash diagnostics, and support bundle collection.
